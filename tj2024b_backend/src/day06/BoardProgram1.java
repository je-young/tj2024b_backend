package day06; // 현재 클래스가 위치한 패키지/폴더명
...
public class BoardProgram1 { // class start // 	자바 모든 코드는 class안에 존재한다.
	...
	public static void main(String[] args) { // main start // 자바의 모든 실행 코드는 main 안에서 시작된다.
		// [6-1] 변수, 기본타입[ boolean char byte short int long float double], 참조타입[String, Scanner 등등]
		...
		while (true) { // [1] 반복문, 무한루프 // while start
			...
			if (선택메뉴 == 1) {
				...
				if ( 제목1 == null ) {
					제목1 = title; 내용1 = content; 작성자1 = writer;	// - while 밖 변수에 입력받은 3개의 문자
				}else if ( 제목2 == null ) {
					제목2 = title; 내용2 = content; 작성자2 = writer;	// 두번째 게시물이 비어 있으면
				}else if ( 제목3 == null ) {
					제목3 = title; 내용3 = content; 작성자3 = writer;	// 세번째 게시물이 비어 있으면
				}else {
					System.out.println("** 등록 실패 : 게시물 작성공간이 부족합니다. **");	// 그외.
				} // if end
				// [6-2] 변수값 수정
				제목1 = title; 내용1 = content; 작성자1 = writer;
				...
				if( 제목1 != null ) {System.out.println("> 제목 : " + 제목1 +" > 내용 : " + " > 작성자 : " + 작성자1 );}
				if( 제목2 != null ) {System.out.println("> 제목 : " + 제목2 +" > 내용 : " + " > 작성자 : " + 작성자2 );}
