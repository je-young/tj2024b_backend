...
=======
        ...
        // 삼항연산자
        String grade = ((score2 >= 90) ? "A등급" : (score2 >= 80) ? "B등급" : (score2 >= 70) ? "C등급" : "재시험");
        // score2 가= 90이상이면? grade 는 A등급 : score2 가= 80이하이면? grade 는B 등급 : score2 가= 70이하이면? grade 는C 등급 : score2 가= 70이하이면? grade 재시험
        System.out.println("삼항연산자 등급: " + grade);
        ...
        // if else
        int max; // 가장 큰 값을 저장할 변수

        if (a > b) { 				// 만약에 a 가 b 보다 크면
			if (a > c) { 			// 그러면 a 가 c 보다도 크면
				max = a; 			// a 가 c 보다도 크면 a 가 가장 큼
			}else { 					// 위의 조건이 모두 아니면
				max = c; 			// 그렇치 않으면 c 가 가장 큼
			} 						
		}else { 						
			if (b > c) {				// b 가 a 보다 크거나 같은 경우
				max = b;				// b 가 c 보다 크면 b 가 가장 큼
			}else {					// 위의 조건이 모두 아니면
				max = c;				// 그렇치 않으면 c 가 가장 큼
			}				
		} // if end
        System.out.println("[문제3] if else : " + max);
        ...
        System.out.print("플레이어2의 입력 (가위: 0, 바위: 1, 보: 2): ");
        
        // Scanner 로 입력받기 : scan.nextInt()
        int player2 = scan.nextInt();
        
        String result = (player1 == player2) ? "무승부" :
                        (player1 == 0 && player2 == 2 || player1 == 1 && player2 == 0 || player1 == 2 && player2 == 1) ?
                        "플레이어1 승리" : "플레이어2 승리";
        System.out.println(result);
