...
public class Example3 { // class start
	...
	public static void main(String[] args) { // main start
		...
		System.out.println("1부터 10까지 합 : " + sum2); // 1부터 10까지 합 : 55
		...
		for (int value = 1; value <= 10; value++) {
			System.out.println("안녕하세요" + value); // 반복되는 코드 : System.out.println("안녕하세요?");
		} // for end
		...
		for (int num = 1; num <= 100; num++) {
			if (num % 2 == 0) {
				...
				continue; // 이후의 문장은 수행하지 않고 for문의 증감식으로 이동한다.
			} // if end
			total += num; // 홀수값들의 누적합계
		} // f end
		...
		int sum3 = 0;
		for (int num = 0;; num++) { // 무한반복
			sum3 += num; // sum3 = sum3 + num
			if (sum3 >= 100) { // 만약에 sum3이 100보다 이상이면
				break; // 반복문 종료/탈출 한다.
			}
		...
		switch (operator) { // 4. operator(연산자) 조건 두고
			case '+':
				...
				break; // 5. 조건이 '+' 이면 더하기 수행
			case '-':
				System.out.println(num1 - num2);
				break; // 5. 조건이 '-' 이면 뺄셈 수행
			case '*':
				System.out.println(num1 * num2);
				break; // 5. 조건이 '*' 이면 곱하기 수행
			case '/':
				System.out.println(num1 / num2);
				break; // 5. 조건이 '/' 이면 나누기 수행
		} // switch end
