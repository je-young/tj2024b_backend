package day06_BoardProgram1_클래스; // 현재 클래스가 위치한 패키지/폴더명
...
public class BoardProgram1 { // class start // 자바 모든 코드는 class안에 존재한다.
	public static void main(String[] args) { // main start // 자바의 모든 실행 코드는 main 안에서 시작된다.
		// [6-1] 변수, 기본타입[ boolean char byte short int long float double], 참조타입[String,
		// Scanner 등등]
		String 제목1 = null; // null 이란? 참조값이 없다는 뜻, 문자열이 없다는 뜻.
		String 내용1 = null;
		String 작성자1 = null;
		String 제목2 = null;
		String 내용2 = null;
		...
		while (true) { // [1] 반복문, 무한루프 // while start
			...
			if (선택메뉴 == 1) {
				...
				System.out.print("> 제목 : ");
				String title = scan.next(); // 메시지(print) -> 입력(next) -> 저장(변수)
				System.out.print("> 내용 : ");
				...
				if (제목1 == null) {
					제목1 = title;
					내용1 = content;
					작성자1 = writer; // - while 밖 변수에 입력받은 3개의 문자
				} else if (제목2 == null) {
					제목2 = title;
					내용2 = content;
					작성자2 = writer; // 두번째 게시물이 비어 있으면
					내용2 = content;
				} else if (제목3 == null) {
					제목3 = title;
					작성자3 = writer; // 세번째 게시물이 비어 있으면
					내용3 = content;
					작성자3 = writer; // 세번째 게시물이 비어 있으면
				} else {
					System.out.println("** 등록 실패 : 게시물 작성공간이 부족합니다. **"); // 그외.
				} // if end
				작성자1 = writer;
				내용1 = content;
				제목1 = title;
				작성자1 = writer;
				System.out.println("** 등록 성공 **"); // - while 밖 변수에 입력받은 3개의 문자열을 각각 저장.
			} else if (선택메뉴 == 2) {
				...
				if (제목3 != null) {
					System.out.println("> 제목 : " + 제목3 + " > 내용 : " + " > 작성자 : " + 작성자3);
				}
				// 오류자 발생하는 이유 : 변수의 초기값(처음에 변수 만들때 값을 넣는 행위-초기화) 없다. // - 지역변수가 초기값이 없으면 호출이
				// 불가는 하다.
			} // if end
			scan.close(); // - 입력객체 닫기
		} // while end
