...
public class Example2 { // class start
	public static void main(String[] args) { // main start
		...
		int 콜라재고 = 10;
		int 사이다재고 = 10;
		int 환타재고 = 10; // 각 제품별 재고 변수에 10 으로 초기화.
		int 콜라바구니 = 0;
		int 사이다바구니 = 0;
		int 환타바구니 = 0; // 각 제품별 장바구니 변수에 0 으로 초기화.
		int 콜라가격 = 1000;
		int 사이다가격 = 1500;
		int 환타가격 = 2000; // 각 제품별 가격 변수에 금액 으로 초기화.

		try (Scanner scan = new Scanner(System.in)) { // [1] 반복문 , while , while(true){} 무한루프
			while (true) {
				System.out.println("===== 키오스크 프로그램 ====="); // [2] 출력함수
				System.out.print(">>> 1.콜라 >>> 2.사이다 >>> 3.환타 >>> 4.결제 >>> 5.종료 : ");
				int choose = scan.nextInt(); // [3] 입력함수
				// [4] 조건문 , if , 입력에 따른 논리연산
				if (choose == 1) {
					System.out.println(">> 콜라 선택 했습니다.");
					// +[8] 만약에 재고가 부족하면 안내하기
					if (콜라재고 == 0) {
					...
					System.out.println(">> 사이다 선택 했습니다.");
					if (사이다재고 == 0) {
