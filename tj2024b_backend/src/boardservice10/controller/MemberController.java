...
public class MemberController {
  ...
  public int signup(MemberDto memberDto) {
    // * 다양한 유효성 검사를 넣을 예정
    // [1] 아이디 길이 검사 : 사용자로부터 입력받은 아이디의 길이가 5미만 이거나 30 초과하면 실패
    ...
    // [4] 이름 길이 검사
    if (memberDto.getMname().length() < 2 || memberDto.getMname().length() > 10) {
      return 3;
    }
  ...
  // 2. 로그인 컨트롤러 메소드
  private int loginMno = 0; // 0이면 비로그인상태 , 0초과이면 로그인성공한 회원번호

  public int getLoginMno() { // 로그인 성공한 회원번호를 반환
    return loginMno;
  }
  ...
  public boolean login(MemberDto memberDto) {
    ...
    if (result > 0) { // 로그인 성공
      loginMno = result; // 로그인 성공시 로그인 저장변수에 로그인성공한 회원번호 저장
      return true;
    } else { // 로그인 실패
      return false;
    }
  ...
  public boolean update(MemberDto memberDto) {
    // + 누구를 수정 할껀지 , 현재 로그인 회원을 수정 하므로 현재 로그인된 회원번호를 dto 담아주기.
