...
public class BoardDao extends Dao {
  ...
  public ArrayList<BoardDto> findAll() {
    ...
    try {
      String sql = "select b.* , c.cname , m.mid from board as b inner join category as c on b.cno = c.cno inner join member m on b.mno = m.mno";
      PreparedStatement ps = conn.prepareStatement(sql);
      ResultSet rs = ps.executeQuery();
      while (rs.next()) {
        // 레코드 : DB테이블의 가로/행 데이터 // 조회된 결과 레코드1개 <---> 인스턴스(DTO) 1개 // 레코드 여러개 <---> []
        // 1개 또는 ArrayList 1개
